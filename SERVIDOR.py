...
PORT = 1024  # Puerto que usa el cliente  (los puertos sin provilegios son > 1023)
...
def send_receive_data(conn, data):
...
def play():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        mensaje1 = "1. principiante : tablerode 9×9 casillas y 10 minas. , 2. avanzado : tablerode 16×16 casillas y 40 minas."
        conn.send(mensaje1.encode('utf-8'))

        opcionREC = conn.recv(1024)
        # decode to unicode string
        strings = str(opcionREC, 'utf8')
        # get the num
        opcion = int(strings)
        ...
        print("elegiste la opcion: ", opcion, " donde tenermos ", dim_size, " filaas y columnas y ", num_bombs,
              "bombas")
        print("\n", dim_size)
        print("\n", num_bombs)
    ...
    #tablero = board.to_json()
    #conn.sendall(tablero.encode())
    ...
    while len(board.dug) < board.dim_size ** 2 - num_bombs:
        ...
        user_input = re.split(',(\\s)*', respuesta)  # '0, 3'
